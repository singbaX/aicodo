...
/*
    Action<int> itemAction =(i) =>
    {
        Debug.WriteLine($"done[{Thread.CurrentThread.ManagedThreadId}]-{i}");
    };
    var queue = new ParallelQueue<int>(itemAction,5);
    foreach (var n in Enumerable.Range(1, 100))
    {
        queue.AddItem(n);
    }
    queue.Sealed();
    var done = queue.Start().Result;
 */
