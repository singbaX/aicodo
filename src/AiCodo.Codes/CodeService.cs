/*
 *  TODO:移除SqlCode加载SqlData的操作，服务模式下，不需要多个配置
 *  CodeService改成注册服务的模式
 *  
 */
...
namespace AiCodo.Codes
{
    ...
    public static class CodeService
    {
        ...
        #region 执行命令
        public static void TryExecuteCommand(string commandName, DynamicEntity args)
        {
            if (_Commands.TryGetValue(commandName.ToLower(), out Action<DynamicEntity> cmd))
            {
                ...
                {
                    "Code".Log($"执行命令[{commandName}]出错：{ex.ToString()}");
                }
            }
            else
            {
