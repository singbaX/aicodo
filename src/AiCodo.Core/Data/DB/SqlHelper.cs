...
@"select C.TABLE_NAME,
        ...
        private static void ResetCommandPageText(SqlItem sql, DbCommand cmd)
        {
            if (sql.CanUsePage)
            {
                ...
                cmd.CommandText = sql.CommandText.TrimEnd(' ', '\r', '\n', ';') + limit;
            }
            else
            {
